 ...
 * 
 * @parameter 텍스트파일 텍스트파일을 음운으로 쪼개기위해 phonemeExtractor 의 split 함수 호출
 */
...
public class PhonemeKernelClassifier implements Serializable {
	...
	public PhonemeKernelClassifier(String dataPath, int subsequenceLength,
			double lambda, boolean debug) throws IOException {
		try {
			this.debug = debug;
			String[] path = dataPath.split("[.]");

			// 음운으로 변환된 arff파일이 아닐 경우, 음운 형태로 변환
			String arffPath = (!path[path.length - 1].equals("arff")) ? convertToPhoneme(dataPath)
					: dataPath;
			svm = createLearnedSVM(arffPath, subsequenceLength, lambda);// svm(smo)
																		// 분류기
																		// 생성.
		} catch (Exception e) {
	...
	private String convertToPhoneme(String arg) throws IOException {
		...
		String path = fi.getPath().substring(0,
				fi.getPath().length() - fi.getName().length());
		...
		fout.println("@attribute 2 {happy,angry,sad,excite,love,hate,expect,natural}");
		fout.println("@data");
		...
		while ((buf = fin.readLine()) != null) {
			...
			String sentance = PhonemeExtractor.split(temp);// 텍스트파일을 음운으로 쪼개기위해
															// phonemeExtractor
															// 의 split 함수 호출
			String sentance2 = PhonemeExtractor.split(temp2);
			// String sentance3 = PhonemeExtractor.split(temp3);

			fout.println("'" + sentance + "'" + "," + "'" + sentance2 + "'"
					+ "," + emotion);
		}
	 ...
	 * 
	 * @parameter arff 파일 입력받은 arff 파일로 svm 분류기를 학습시킨다.
	 */
	protected SMO createLearnedSVM(String dataset, int subsequenceLength,
		...
		StringKernel kernel = new StringKernel(data, 250007, subsequenceLength,
				lambda, debug);
